...
export default function Dashboard() {
  ...
  const animate = settings.showAnimations;
  ...
  // Motion wrapper — returns motion.div when animations enabled, plain div otherwise
  ...
  return (
    ...
    <div className="flex flex-col relative w-full h-screen font-sans text-[#E4E1EE] overflow-hidden">
      ...
      <div className="relative z-10 w-full h-full overflow-y-auto overflow-x-hidden custom-scrollbar pb-24">
        <motion.div 
          className="max-w-5xl mx-auto space-y-10 px-4 md:px-8 py-8 md:py-12"
        ...
        >
          ...
          {/* HERO SECTION */}
          <MotionSection 
            className="relative w-full overflow-hidden rounded-[2rem] border border-red-500/50 bg-transparent shadow-[0_0_40px_rgba(225,6,0,0.15)] backdrop-blur-[2px]" 
            style={{ minHeight: '440px' }}
            {...(animate ? { variants: sectionVariant } : {})}
          >
              ...
              <div>
                <div className="flex items-center space-x-3 mb-6">
                  ...
                  <span className="text-white text-sm md:text-base font-bold flex items-center gap-2 drop-shadow-lg">
                    <span>{latestRace?.flag || '🏁'}</span> GRAND PRIX {latestRace?.country?.toUpperCase() || ''}
                  </span>
                </div>
                <h1 className="font-headline font-black text-[3.5rem] md:text-7xl lg:text-[5.5rem] leading-[0.9] italic uppercase tracking-[-0.04em] max-w-2xl text-white drop-shadow-2xl">
          ...
          {latestRace && (
            <MotionSection 
              className="w-full bg-transparent border border-white/20 rounded-[2rem] p-6 lg:p-8 backdrop-blur-[2px] shadow-lg"
              {...(animate ? { variants: sectionVariant } : {})}
            >
              ...
              <div className="flex flex-col space-y-3">
                {latestRace.results.slice(0, 4).map((result, idx) => (
                  <motion.div 
                    key={result.driverId} 
                    className={`flex items-center justify-between p-4 px-6 rounded-xl bg-black/10 backdrop-blur-sm border ${idx === 0 || idx === 1 ? 'border-red-500/60 shadow-[0_0_20px_rgba(225,6,0,0.15)]' : 'border-white/20'} hover:border-white/50 transition-all`}
                    {...(animate ? {
                      initial: { opacity: 0, x: -16 },
                      animate: { opacity: 1, x: 0 },
                      transition: { type: 'spring', stiffness: 300, damping: 28, delay: idx * 0.06 },
                    } : {})}
                  >
          ...
          >
            ...
            <motion.div 
              ...
              variants={animate ? cardStagger : undefined}
              initial={animate ? "hidden" : undefined}
              animate={animate ? "visible" : undefined}
            >
              {timelineRaces.map((race) => {
                ...
                return (
                  <motion.div 
                    ...
                    whileHover={animate ? { scale: 1.025, transition: { type: 'spring', stiffness: 400, damping: 20 } } : undefined}
                  >
                    <Link 
              ...
              {topDrivers.map((driver, idx) => {
                return (
                  <motion.div
                    key={driver.id}
                    variants={animate ? cardVariant : undefined}
                    whileHover={animate ? { scale: 1.02, transition: { type: 'spring', stiffness: 400, damping: 18 } } : undefined}
                  >
                    <Link to={`/driver/${driver.id}`} className="group hover-glow-flare relative overflow-hidden rounded-3xl bg-transparent border border-white/20 block h-[380px] backdrop-blur-sm">
